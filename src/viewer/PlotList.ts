...
module ChartViewer {
    export function PlotList(rootDiv, plotViewer, persistentViewState, transientViewState) {
        ...
        var legend = $("<div></div>").appendTo(rootDiv);
        var leg = new InteractiveDataDisplay.Legend(plotViewer.iddChart, legend);
        ...
        //var createDefaultCardContent = function (plotInfo: PlotInfo) {
        //    var content = $("<div></div>").text(plotInfo.kind);
        //    for (var prop in plotInfo) {
        //        if (prop !== "kind") {
        //            var propVal = getTitle(plotInfo, prop);
